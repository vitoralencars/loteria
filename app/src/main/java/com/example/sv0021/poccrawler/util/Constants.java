package com.example.sv0021.poccrawler.util;

public class Constants {

    public static final String BASE_API_URL = "http://172.16.46.21:3000/";

    public static final String SHARED_PREFS_TAG = "Shared_Prefs_Tag";
    public static final String SHARED_PREFS_JOGOS_MEGA_SENA = "Shared_Prefs_Jogos_Mega_Sena";
    public static final String SHARED_PREFS_JOGOS_LOTOFACIL = "Shared_Prefs_Jogos_Lotofacil";
    public static final String SHARED_PREFS_JOGOS_QUINA = "Shared_Prefs_Jogos_Quina";
    public static final String SHARED_PREFS_JOGOS_LOTOMANIA = "Shared_Prefs_Jogos_Lotomania";

    public static final String EXTRA_LOTERIA = "extra_loteria";

}
